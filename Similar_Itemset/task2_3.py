...
def main(folder_path, test_path, output_path):
    ...
    '''
    # Classify which CF to use
    start_time = (time.time())
    with open("task2_3_train/model.md","rb") as file:
        selection_model = pickle.load(file)
    selection_label = selection_model.predict(X_test)
    selection_map = {pair:m for pair,m in zip(test_pairs,selection_label)}
    print(time.time()-start_time)
    '''
    ...
    mb_result = [(pair,pred) for pair,pred in zip(test_pairs,y_pred) if selection_map[pair]==1]
    print(time.time()-start_time)
    '''    
